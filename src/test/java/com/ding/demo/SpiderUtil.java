...
public class SpiderUtil {
    ...
    public static void main(String args[]) {
        String url = "https://mp.weixin.qq.com/s/b7kwtYTLjCuQf7rtX2SmcQ";
...
//            System.out.println(html);
    ...
    public  void convertImg(Element element,JSONArray tags) {
        JSONObject tag = new JSONObject(new LinkedHashMap<>());
        String tagName = element.tagName();
        tag.put(KEY_NAME, tagName);
        switch (tagName) {
    ...
    public static void parseContent(Element element, JSONArray tags, StringBuilder s) {
        JSONObject tag = new JSONObject(new LinkedHashMap<>());
        String tagName = element.tagName();
        tag.put(KEY_NAME, tagName);
        switch (tagName) {
...
//                tag.put(KEY_TEXT, element.text());
//                tags.add(tag);
                s.append("<span>").append(element.text()).append("</span>");
                break;
            }
            case "img": {
                s.append("<img");
                ...
                if (attrs != null) {
                    for (Attribute attr : attrs) {
                        String atrKey = attr.getKey().replace("-", "");
                        /*if(atrKey.equalsIgnoreCase("datasrc")){
                            atrKey = "src";
                        }*/
                        if (atrKey.startsWith("data")) {
                            atrKey = atrKey.replace("data", "");
                        }
                        String attrString = StringUtils.isEmpty(attr.getValue()) ? "''" : "'" + attr.getValue() + "'";
                        s.append(" ").append(atrKey).append("=").append(attrString);
                    }
                }
                s.append("></img>");
//                tags.add(tag);
...
//                tag.put(KEY_TEXT, element.attr("textvalue"));
                ...
                s.append("<a ")
                        .append(" href=").append(StringUtils.isEmpty(element.attr("href")) ? "''" : "'" + element.attr("href") + "'")
                        .append(" textvalue=").append(StringUtils.isEmpty(element.attr(" textvalue=")) ? "''" : "'" + element.attr(" textvalue=") + "'")
                        .append(" ")
                        .append("</a>");
//                tags.add(tag);
                break;
...
//                tag.put(KEY_TEXT, element.text());
//                tags.add(tag);
                s.append("<p>").append(element.text()).append("</p>");
                break;
            }
            default:
                break;
